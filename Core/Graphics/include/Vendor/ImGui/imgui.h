...
typedef unsigned ImGuiID;       // Unique ID used by widgets (typically hashed from a stack of string)
...
typedef unsigned        ImU32;  // 32-bit unsigned integer (often used to store packed colors)
...
namespace ImGui
{
    ...
    IMGUI_API bool          CheckboxFlags(const char* label, unsigned* flags, unsigned flags_value);
    ...
    IMGUI_API void          Value(const char* prefix, unsigned v);
...
struct ImDrawCmd
{
    unsigned    ElemCount;              // Number of indices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtx_buffer[] array, indices in idx_buffer[].
...
// Vertex index (override with '#define ImDrawIdx unsigned' inside in imconfig.h)
#ifndef ImDrawIdx
...
struct ImDrawList
{
    ...
    unsigned            _VtxCurrentIdx;     // [Internal] == VtxBuffer.Size
...
struct ImFontConfig
{
    ...
    unsigned    RasterizerFlags;        // 0x00     // Settings for custom font rasterizer (e.g. ImGuiFreeType). Leave as zero if you aren't using one.
...
struct ImFontAtlas
{
    ...
    struct CustomRect
    {
        unsigned    ID;             // Input    // User ID. Use <0x10000 to map into a font glyph, >=0x10000 for other/internal/custom texture data.
    ...
    IMGUI_API int       AddCustomRectRegular(unsigned id, int width, int height);                                                                   // Id needs to be >= 0x10000. Id >= 0x80000000 are reserved for ImGui and ImDrawList
    ...
    unsigned*               TexPixelsRGBA32;    // 4 component per pixel, each component is unsigned 8-bit. Total size = TexWidth * TexHeight * 4
