...
void ImDrawList::AddPolyline(const ImVec2* points, const int points_count, ImU32 col, bool closed, float thickness)
{
    ...
    if (Flags & ImDrawListFlags_AntiAliasedLines)
    {
        ...
        if (!thick_line)
        {
            ...
            unsigned idx1 = _VtxCurrentIdx;
            for (int i1 = 0; i1 < count; i1++)
            {
                const int i2 = (i1+1) == points_count ? 0 : i1+1;
                unsigned idx2 = (i1+1) == points_count ? _VtxCurrentIdx : idx1+3;
        ...
        else
        {
            ...
            {
                const int i2 = (i1+1) == points_count ? 0 : i1+1;
                unsigned idx2 = (i1+1) == points_count ? _VtxCurrentIdx : idx1+4;
...
void ImDrawList::AddConvexPolyFilled(const ImVec2* points, const int points_count, ImU32 col)
{
    ...
    if (Flags & ImDrawListFlags_AntiAliasedFill)
    {
        ...
        unsigned vtx_inner_idx = _VtxCurrentIdx;
        unsigned vtx_outer_idx = _VtxCurrentIdx+1;
...
const unsigned FONT_ATLAS_DEFAULT_TEX_DATA_ID = 0x80000000;
...
void    ImFontAtlas::GetTexDataAsRGBA32(unsigned char** out_pixels, int* out_width, int* out_height, int* out_bytes_per_pixel)
{
    ...
    if (!TexPixelsRGBA32)
    {
        ...
        if (pixels)
        {
            TexPixelsRGBA32 = (unsigned*)ImGui::MemAlloc((size_t)(TexWidth * TexHeight * 4));
            const unsigned char* src = pixels;
            unsigned* dst = TexPixelsRGBA32;
            for (int n = TexWidth * TexHeight; n > 0; n--)
                *dst++ = IM_COL32(255, 255, 255, (unsigned)(*src++));
        }
    }
...
static unsigned stb_decompress_length(const unsigned char *input);
static unsigned stb_decompress(unsigned char *output, const unsigned char *input, unsigned length);
static const char*  GetDefaultCompressedFontDataTTFBase85();
static unsigned Decode85Byte(char c)                                    { return c >= '\\' ? c-36 : c-35; }
static void         Decode85(const unsigned char* src, unsigned char* dst)
{
    while (*src)
    {
        unsigned tmp = Decode85Byte(src[0]) + 85*(Decode85Byte(src[1]) + 85*(Decode85Byte(src[2]) + 85*(Decode85Byte(src[3]) + 85*Decode85Byte(src[4]))));
...
{
    const unsigned buf_decompressed_size = stb_decompress_length((const unsigned char*)compressed_ttf_data);
    unsigned char* buf_decompressed_data = (unsigned char *)ImGui::MemAlloc(buf_decompressed_size);
    stb_decompress(buf_decompressed_data, (const unsigned char*)compressed_ttf_data, (unsigned)compressed_ttf_size);
...
int ImFontAtlas::AddCustomRectRegular(unsigned id, int width, int height)
{
...
void    ImFontAtlasBuildMultiplyCalcLookupTable(unsigned char out_table[256], float in_brighten_factor)
{
    ...
    {
        unsigned value = (unsigned)(i * in_brighten_factor);
...
void ImFont::RenderText(ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width, bool cpu_fine_clip) const
{
    ...
    unsigned vtx_current_idx = draw_list->_VtxCurrentIdx;
    ...
    draw_list->_VtxCurrentIdx = (unsigned)draw_list->VtxBuffer.Size;
}
...
static void stb__match(const unsigned char *data, unsigned length)
{
...
static void stb__lit(const unsigned char *data, unsigned length)
{
    IM_ASSERT(stb__dout + length <= stb__barrier_out_e);
    if (stb__dout + length > stb__barrier_out_e) { stb__dout += length; return; }
...
static unsigned stb_adler32(unsigned adler32, unsigned char *buffer, unsigned buflen)
{
...
static unsigned stb_decompress(unsigned char *output, const unsigned char *i, unsigned /*length*/)
{
    unsigned olen;
    ...
    for (;;) {
        ...
        if (i == old_i) {
            if (*i == 0x05 && i[1] == 0xfa) {
                ...
                if (stb_adler32(1, output, olen) != (unsigned) stb__in4(2))
                    return 0;
                return olen;
            } else {
