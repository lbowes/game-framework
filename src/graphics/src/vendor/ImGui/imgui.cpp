...
/*
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       font init:  const void* png_data; unsigned png_size; ImGui::GetDefaultFontData(NULL, NULL, &png_data, &png_size); <..Upload texture to GPU..>
...
static bool             InputTextFilterCharacter(unsigned* p_char, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data);
...
int ImTextCharFromUtf8(unsigned* out_char, const char* in_text, const char* in_text_end)
{
    ...
    if ((*str & 0xf0) == 0xe0)
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        ...
        c = (unsigned)((*str++ & 0x0f) << 12);
        if ((*str & 0xc0) != 0x80) return 3;
        c += (unsigned)((*str++ & 0x3f) << 6);
        if ((*str & 0xc0) != 0x80) return 3;
        c += (*str++ & 0x3f);
        *out_char = c;
        return 3;
    ...
    if ((*str & 0xf8) == 0xf0)
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        ...
        c = (unsigned)((*str++ & 0x07) << 18);
        if ((*str & 0xc0) != 0x80) return 4;
        c += (unsigned)((*str++ & 0x3f) << 12);
        if ((*str & 0xc0) != 0x80) return 4;
        c += (unsigned)((*str++ & 0x3f) << 6);
        if ((*str & 0xc0) != 0x80) return 4;
        c += (*str++ & 0x3f);
...
static inline int ImTextCharToUtf8(char* buf, int buf_size, unsigned c)
{
    if (c < 0x80)
    {
        buf[0] = (char)c;
...
static inline int ImTextCountUtf8BytesFromChar(unsigned c)
{
    if (c < 0x80) return 1;
    if (c < 0x800) return 2;
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    // B) If you need/want meshes with more than 64K vertices, uncomment the '#define ImDrawIdx unsigned' line in imconfig.h to set the index size to 4 bytes. 
...
static void DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, void* arg1, const void* arg2)
{
    ...
    switch (data_type)
    {
        ...
        case ImGuiDataType_U32:
            if (op == '+')      *(unsigned*)output = *(const unsigned*)arg1 + *(const ImU32*)arg2;
            else if (op == '-') *(unsigned*)output = *(const unsigned*)arg1 - *(const ImU32*)arg2;
            return;
...
const char* ImParseFormatFindEnd(const char* fmt)
{
    ...
    if (fmt[0] != '%')
        return fmt;
    const unsigned ignored_uppercase_mask = (1 << ('I'-'A')) | (1 << ('L'-'A'));
    const unsigned ignored_lowercase_mask = (1 << ('h'-'a')) | (1 << ('j'-'a')) | (1 << ('l'-'a')) | (1 << ('t'-'a')) | (1 << ('w'-'a')) | (1 << ('z'-'a'));
...
bool ImGui::CheckboxFlags(const char* label, unsigned* flags, unsigned flags_value)
{
...
static bool is_separator(unsigned c)                                        { return ImCharIsBlankW(c) || c==',' || c==';' || c=='(' || c==')' || c=='{' || c=='}' || c=='[' || c==']' || c=='|'; }
        ...
        if (io.InputCharacters[0])
        {
            ...
            if (!ignore_inputs && is_editable && !user_nav_input_start)
                for (int n = 0; n < IM_ARRAYSIZE(io.InputCharacters) && io.InputCharacters[n]; n++)
                {
                    ...
                    unsigned c = (unsigned)io.InputCharacters[n];
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (allow_opt_alpha_bar)
    {
        ...
        ImGui::CheckboxFlags("Alpha Bar", (unsigned*)&g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
    }
    ImGui::EndPopup();
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    else if ((flags & ImGuiColorEditFlags_HEX) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (InputText("##Text", buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            ...
            i[0] = i[1] = i[2] = i[3] = 0;
            if (alpha)
                sscanf(p, "%02X%02X%02X%02X", (unsigned*)&i[0], (unsigned*)&i[1], (unsigned*)&i[2], (unsigned*)&i[3]); // Treat at unsigned (%X is unsigned)
            else
                sscanf(p, "%02X%02X%02X", (unsigned*)&i[0], (unsigned*)&i[1], (unsigned*)&i[2]);
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
        PopItemWidth();
...
void ImGui::Value(const char* prefix, unsigned v)
{
    Text("%s: %d", prefix, v);
}
