...
template<>
struct std::hash<Framework::Utils::IndexSet>
{
	const size_t operator()(const Framework::Utils::IndexSet& key) const
	{
...
namespace Framework {
	namespace Utils {
		...
		void OBJLoader::loadOBJData(ResourceSet& dataHolder, Graphics::Mesh* target, std::string filePath) {
			...
			for (std::string line : lines) {
				...
				if (strstr(cstr, "#") || strstr(cstr, "s"))
					...
					if (strstr(cstr, "vt")) {
						glm::vec2 texCoord;
						int result = sscanf_s(cstr, "%*s %f %f", &texCoord.x, &texCoord.y);
						if (result == 0)
							continue;
					...
					if (faceVertices.size() > 3) {
						ConsoleLogger::log(ConsoleLogger::LogType::ERROR_HALT, "OBJ model " + filePath + " was not exported with triangulated faces.");
						return;
					}
			...
			//ConsoleLogger::log(ConsoleLogger::LogType::INFO, "OBJ Mesh loaded from '" + filePath + ".\n\n");
			...
			if (containsPositions) {
				dataHolder.addVertexBuffer(GL_STATIC_DRAW, VertexFormat(0, 3, GL_FLOAT, false), extractAttribData(vertices, AttribType::POSITION), target->getUniqueID() + ":POSITIONS");
				target->addBuffer(dataHolder.getResource<VertexBuffer>(target->getUniqueID() + ":POSITIONS"));
			}
			
			if (containsTexCoords) {
				dataHolder.addVertexBuffer(GL_STATIC_DRAW, VertexFormat(1, 2, GL_FLOAT, false), extractAttribData(vertices, AttribType::TEXTURE_COORD), target->getUniqueID() + ":TEX_COORDS");
				target->addBuffer(dataHolder.getResource<VertexBuffer>(target->getUniqueID() + ":TEX_COORDS"));
			}
			
			if (containsNormals) {
				dataHolder.addVertexBuffer(GL_STATIC_DRAW, VertexFormat(2, 3, GL_FLOAT, false), extractAttribData(vertices, AttribType::NORMAL), target->getUniqueID() + ":NORMALS");
				target->addBuffer(dataHolder.getResource<VertexBuffer>(target->getUniqueID() + ":NORMALS"));
			}
			
			dataHolder.addIndexBuffer(GL_STATIC_DRAW, indices, target->getUniqueID() + ":INDICES");
			target->addIndexBuffer(dataHolder.getResource<IndexBuffer>(target->getUniqueID() + ":INDICES"));
		}

		void OBJLoader::insertVertex(std::vector<Vertex>& vertices, std::vector<unsigned int>& indices, std::unordered_map<IndexSet, int>& mapping, VertexSet& inputVertices, IndexSet& indexSet) {
			auto lookup = mapping.find(indexSet);

			if (lookup != mapping.end())
				indices.push_back(lookup->second);
			else {
				mapping[indexSet] = vertices.size();
