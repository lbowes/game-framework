...
namespace Framework {
	namespace Utils {
		...
		unsigned int Timer::mLatestEventDuration, Timer::mResolution;
		std::vector<TimedEvent> Timer::mTimedEvents;

		void Timer::start(unsigned int resolution) {
			mResolution = resolution;
			mStartTime = std::chrono::high_resolution_clock::now();
		}

		void Timer::stop(std::string eventName, bool addToEvents) {
		...
		void Timer::addTimedEvent(std::string eventName) {
