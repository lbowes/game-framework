...
namespace Framework {
	namespace Maths {
		...
		std::vector<unsigned int> Noise::p;
		bool Noise::permutationsInitialised = false;

		unsigned int Noise::permutation[256] = { 151,160,137,91,90,15,			
		...
		double Noise::multiFractalBillowed(double x, double z, double lacunarity, double gain, double octaves) {
			...
			return sum;
#else
			float perlinResult = perlin(x, z);
			float sum = perlinResult;
			float amp = 1.0f;
